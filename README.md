...
|
 Auth 
|
 JWT (jjwt 0.12.x) + BCrypt (12 rounds) 
|
|
 Mapping 
|
...
|
`jdbc:postgresql://localhost:5433/buildflow`
|
|
`DB_USERNAME`
|
...
`PATCH`
|
`/{id}/validate-bl?bonLivraisonRef=BL-001`
|
`ADMIN`
, 
`ACHAT`
...
`PM`
|
 Confirm delivery → Stock provisioned 
|
|
`PATCH`
|
`/{id}/validate-facture?factureRef=FA-001`
|
`ADMIN`
, 
`FINANCE`
...
`CHEF_CHANTIER`
|
 Get inventory per site (paginated) 
|
**Response:**
```json
{
...
`CHEF_CHANTIER`
|
 Get by ID (includes transactions) 
|
|
`POST`
|
...
`FINANCE`
|
 Payments, treasury, payroll validation 
|
|
`RH`
|
 Employees, payroll creation 
|
|
`PM`
|
 Project management, some purchasing 
|
|
`ACHAT`
|
 Purchasing, articles, suppliers 
|
|
`CHEF_CHANTIER`
|
...
|
 Duplicate 
|
`"A fiche de paie already exists for this employee for period 2026-07"`
|
|
`422`
|
 Business rule 
|
`"Insufficient funds in caisse 'CAISSE-CH001'. Solde: 5000.00, Debit: 9000.00"`
|
|
`500`
|
